...
urlHandler Anilist (Just r) = do
    ...
    throwError $ err302 { errHeaders =
        [("Location", B8.pack $ "https://anilist.co/api/v2/oauth/authorize?client_id=" ++ clientId ++ "&response_type=code&redirect_uri=http://localhost:8080/auth/redirect&&state=" ++ r)] } 
urlHandler Discord (Just r) = do
    clientId <- liftIO $ envAsString "DISCORD_CLIENT_ID" ""
    throwError $ err302 { errHeaders =
        [("Location", B8.pack $ "https://discord.com/api/oauth2/authorize?response_type=code&scope=identify&client_id=" ++ clientId ++ "&response_type=code&redirect_uri=http://localhost:8080/auth/redirect&&state=" ++ r)] } 
urlHandler Google  (Just r) = do
    clientId <- liftIO $ envAsString "GOOGLE_CLIENT_ID" ""
    throwError $ err302 { errHeaders =
        [("Location", B8.pack $ "https://accounts.google.com/o/oauth2/v2/auth?scope=https://www.googleapis.com/auth/youtube.force-ssl&access_type=offline&prompt=consent&include_granted_scopes=true&response_type=code&client_id=" ++ clientId ++ "&redirect_uri=http://localhost:8080/auth/redirect&state=" ++ r)] } 
urlHandler Twitter (Just r) = do
    clientId <- liftIO $ envAsString "TWITTER_CLIENT_ID" ""
    throwError $ err302 { errHeaders =
        [("Location", B8.pack $ "https://twitter.com/i/oauth2/authorize?response_type=code&scope=like.write like.read follows.read follows.write offline.access tweet.read tweet.write&code_challenge=challenge&code_challenge_method=plain&client_id=" ++ clientId ++ "&redirect_uri=http://localhost:8080/auth/redirect&state=" ++ r)] } 
urlHandler Spotify (Just r) = do
    clientId <- liftIO $ envAsString "SPOTIFY_CLIENT_ID" ""
    throwError $ err302 { errHeaders =
        [("Location", B8.pack $ "https://accounts.spotify.com/authorize?response_type=code&scope=user-library-read&client_id=" ++ clientId ++ "&redirect_uri=http://localhost:8080/auth/redirect&&state=" ++ r)] } 
urlHandler Github  (Just r) = do
    clientId <- liftIO $ envAsString "GITHUB_CLIENT_ID" ""
    throwError $ err302 { errHeaders =
        [("Location", B8.pack $ "https://github.com/login/oauth/authorize?response_type=code&scope=repo&client_id=" ++ clientId ++ "&redirect_uri=http://localhost:8080/auth/redirect&&state=" ++ r)] } 
